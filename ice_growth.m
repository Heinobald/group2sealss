function ice_growth = ice_growth(sal,time,time_frame)
%calculates ice growth from salinity
%   sal(depth,time) in psu (enter 2D array)
%   time in days (enter array)
%   time_frame in days
...
h0=100;
ice_growth=sal*time_frame*time;
    % for t=1:(length(time))-time_frame
    %         S_0=sum(sal(3:21,t));
    %         S_f=sum(sal(3:21,t+time_frame));
    %         ice_growth(t) = ((rho_0*h0*S_0-rho_0*h0*S_f)/(rho_i*S_i-rho_0*S_f))/(time(t+time_frame)-time(t));
    % end
